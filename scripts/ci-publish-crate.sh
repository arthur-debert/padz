...
# the heuristic (the latest != the older being re-published). The two-tier
# check here — JSON API pre-check + grep on cargo's own publish output —
# correctly tolerates re-runs of any version.
