...
fn main() {
    ...
    // Global root: Use XDG
    let proj_dirs = ProjectDirs::from("com", "padz", "padz").expect("Could not determine config dir");
    ...
    let scope = if cli.global { Scope::Global } else { Scope::Project };

    // Handle commands
    let result = match cli.command {
        Some(Commands::Create { title, content }) => {
            let content_str = content.unwrap_or_default();
            // TODO: Read stdin if piped
            match api.create_pad(title, content_str, scope) {
                 Ok(pad) => {
        ...
        Some(Commands::View { index }) => {
            ...
            match api.get_pad(&idx, scope) {
                Ok(dp) => {
                    println!("{} {}", dp.index.to_string().yellow(), dp.pad.metadata.title.bold());
                    println!("--------------------------------");
        ...
        Some(Commands::Search { term }) => {
            match api.search_pads(&term, scope) {
                Ok(pads) => {
                    print_pads(&pads);
                    Ok(())
                }
...
fn print_pads(pads: &[DisplayPad]) {
    ...
    for dp in pads {
        ...
        let pin_icon = if dp.pad.metadata.is_pinned { "📌 " } else { "" };
