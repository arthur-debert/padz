...
- **Added**
  ...
  - **Homebrew distribution** — padz is now published to the `arthur-debert/homebrew-padz` tap and intended to be installable via `brew install arthur-debert/padz/padz` on macOS (Apple Silicon + Intel) and Linux (x86_64 + ARM64). The formula is regenerated on every release from `.github/homebrew-formula.rb.tmpl` and pushed to the tap automatically. It also installs bash/zsh/fish completions into Homebrew's standard completion directories for brew-managed setups.
