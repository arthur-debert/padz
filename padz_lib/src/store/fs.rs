...
impl FileStore {
    ...
    fn load_metadata(&self, store_path: &Path) -> Result<HashMap<Uuid, Metadata>> {
        ...
        let meta: HashMap<Uuid, Metadata> = serde_json::from_str(&content).map_err(PadzError::Serialization)?;
        Ok(meta)
    }
...
impl DataStore for FileStore {
    ...
    fn get_pad(&self, id: &Uuid, scope: Scope) -> Result<Pad> {
        ...
        let metadata = meta_map.get(id).ok_or_else(|| PadzError::PadNotFound(*id))?.clone();
